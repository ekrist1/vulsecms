...
describe('users service', () => {
  ...
  it('lists with role filter and pagination', async () => {
    await createUser(adapter, { email: 'a@x.com', password: 'hunter2hunter2', role: 'editor', isSuper: false, name: 'A' });
    await createUser(adapter, { email: 'b@x.com', password: 'hunter2hunter2', role: 'external_user', isSuper: false, name: 'B' });
  ...
  it('updates name, role, is_super, group memberships', async () => {
    const u = await createUser(adapter, { email: 'a@x.com', password: 'hunter2hunter2', role: 'editor', isSuper: false, name: 'A' });
  ...
  it('rejects duplicate emails', async () => {
    await createUser(adapter, { email: 'a@x.com', password: 'hunter2hunter2', role: 'editor', isSuper: false, name: 'A' });
    await expect(
      createUser(adapter, { email: 'a@x.com', password: 'hunter2hunter2', role: 'editor', isSuper: false, name: 'X' }),
    ).rejects.toThrow();
  });
});
