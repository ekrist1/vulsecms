...
describe('protected entries', () => {
  ...
  beforeEach(async () => {
    ...
    const e2 = ulid();
    await db.exec(
      `INSERT INTO entries (id, collection_handle, sort_order, status, protected, content) VALUES (?, 'posts', 1, 'published', 0, ?)`,
      [e1, JSON.stringify({ title: 'Public' })],
    );
    await db.exec(
      `INSERT INTO entries (id, collection_handle, sort_order, status, protected, content) VALUES (?, 'posts', 2, 'published', 1, ?)`,
      [e2, JSON.stringify({ title: 'Secret' })],
    );
  ...
  it('anonymous single GET on protected entry returns 401', async () => {
    const rows = await db.query<{ id: string; protected: number }>(
      'SELECT id, protected FROM entries WHERE collection_handle = ? ORDER BY sort_order',
      ['posts'],
    );
  ...
  it('signed-in super sees protected entry in list and single', async () => {
    ...
    const protectedId = rows.find((r) => r.protected === 1)!.id;
    const single = await app.request(`http://x/api/collections/posts/${protectedId}`, {
      headers: { cookie },
    });
